...
impl PipelineStrategy for GradleStrategy {
    ...
    fn steps(&self, info: &ProjectInfo) -> Vec<Box<dyn StepDef>> {
        ...
        // Build
        steps.push(Box::new(GradleCachedStep::wrap(Box::new(BuildStep::new(info)))));

        // Checkstyle (if lint_cmd present)
        if info.lint_cmd.is_some() {
            steps.push(Box::new(GradleCachedStep::wrap(Box::new(checkstyle_step::CheckstyleStep::new(info)))));
        ...
        if info.quality_plugins.contains(&"spotbugs".to_string()) {
            steps.push(Box::new(GradleCachedStep::wrap(Box::new(spotbugs_step::SpotbugsStep::new(info)))));
        ...
        if quality_step_names.is_empty() {
            ...
            steps.push(Box::new(GradleCachedStep::wrap_with_deps(Box::new(test_step), quality_step_names)));
        }
...
mod tests {
    ...
    fn test_gradle_steps_with_lint() {
        ...
        assert_eq!(names, vec!["build", "checkstyle", "spotbugs", "pmd", "test"]);
        // test depends on quality steps
    ...
    fn test_parse_gradle_test_with_failures() {
        ...
        assert_eq!(parse_gradle_test(output).unwrap(), "17 passed, 3 failed, 0 skipped");
    }

    #[test]
    fn test_parse_gradle_test_with_skipped() {
        let output = "15 tests completed, 2 failed, 3 skipped";
        assert_eq!(parse_gradle_test(output).unwrap(), "10 passed, 2 failed, 3 skipped");
    }
